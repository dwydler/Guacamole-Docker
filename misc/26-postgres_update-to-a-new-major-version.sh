...
# Check if the specified file exists. If the file exists, it will be deleted.
if [ -f "$APPPATH/postgres/init/initdb.sql" ]; then
    rm "$APPPATH/postgres/init/initdb.sql"
fi

# Stops containers and removes containers, networks, volumes, and images created by up
docker compose -f "$APPPATH/docker-compose.yml" down guacamole guacd
...
# Stops the named container and remove container, network, volume, and image created by up
docker compose -f "$APPPATH/docker-compose.yml" down postgres
...
#
cat "$APPPATH/postgres/backup/$FILENAME" | docker exec -i guacamole_database psql -U $POSTGRES_USER
